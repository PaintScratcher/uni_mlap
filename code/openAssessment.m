...
logLikelihoodStore = []; % Store the loglikelihood at each stage so we can plot it later
...
while 1 % Loop until we reach convergence
    ...
    for row = 1:size(datacsv,1) % For each data point
        ...
        logLikelihoodStore(iteration) = totalLogLikelihood; % Store this iterations LL for use in a later plot
    end
    ...
    if abs(totalLogLikelihood - previousLogLikelihood) < 0.0001 % If the difference between the current and previous LL is small enough, we have converged
        ...
        figure % Plot a graph for log-likelihood against iteration number
